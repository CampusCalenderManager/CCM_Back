...
public class CreateScheduleDto {
    ...
    public Schedule toEntity() {
        return Schedule.builder().title(title).startDate(startDate).endDate(endDate).startAlarm(startAlarm).endAlarm(endAlarm).isShared(isShared).color(color).member(new Member(memberId)).build();
    }
}
